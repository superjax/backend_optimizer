    ...
    def calc_jacobian_of_string_with_reversed_edges(self, z, dirs):
        ...
        for i in range(len(dirs)):
            ...
            if dirs[i] > 0:
                ...
                H[0:2, 3*i:3*i + 2] = -self.R(total_angle_so_far + theta[i])
...
if __name__ == '__main__':
    ...
    Omegas = [np.diag([100., 10., 100.]) for i in range(perfect_edges.shape[1])]
    ...
    lc_omega = [np.diag([1000, 1000, 100]) for i in range(lc.shape[1])]
    ...
    for iter in range(num_iters):
        ...
        edges, diff = optimizer.optimize(edges, dirs, Omegas, lc, lc_omega, lc_dir, cycles, 10, 0.000001)
    plt.show()
